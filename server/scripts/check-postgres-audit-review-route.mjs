...
assert(/auditLogId: postgresId\(\)/.test(source), 'audit review creates uuid audit log id');
assert(/adminActionLogId: postgresId\(\)/.test(source), 'audit review creates uuid admin action id');
...
console.log(
  JSON.stringify(
    {
      ok: true,
      checks: ['audit-review-route-gateway', 'uuid-audit-log-id', 'uuid-admin-action-id', 'no-json-save'],
    },
    null,
    2,
  ),
