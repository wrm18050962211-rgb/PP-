...
assert(/conversationId:\s*postgresId\(\)/.test(source), 'wechat payment notify uses uuid conversation id');
assert(/statusLogId:\s*postgresId\(\)/.test(source), 'wechat payment notify uses uuid status log id');
assert(/statusLogId:\s*status === 'succeeded' \? postgresId\(\) : undefined/.test(source), 'wechat refund notify uses uuid status log id');
