...
assert(/conversationId: id\('conversation'\)/.test(source), 'postgres payment route creates conversation id');
assert(/return json\([\s\S]*conversation: createConversation\(paidOrder\)[\s\S]*200,\s*false/.test(source), 'postgres payment route avoids json save');

console.log(
  JSON.stringify(
    {
      ok: true,
      checks: ['payment-route-gateway', 'conversation-id', 'no-json-save'],
    },
    null,
    2,
  ),
