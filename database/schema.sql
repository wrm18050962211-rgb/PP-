...
create type user_request_actor_type as enum (
  'user',
  'admin',
  'system'
);
...
create table reports (
  ...
  report_context report_context not null default 'order',
  order_id uuid references orders(id),
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint ck_reports_request_idempotency_pair
    check (
  ...
  constraint ck_reports_store_lite_content_shape
    check (
...
create unique index uq_reports_store_lite_reporter_client
on reports(reporter_id, client_request_id)
where report_context = 'store_lite_content';

create index idx_reports_store_lite_reporter_created
on reports(reporter_id, report_context, created_at desc, id desc);

create index idx_reports_store_lite_operator_queue
...
create table user_companion_blocks (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references users(id) on delete cascade,
  companion_id uuid not null references companions(id) on delete cascade,
  created_at timestamptz not null default now(),
...
create index idx_user_companion_blocks_user_created
...
-- =========================
-- Store Lite support, data-rights, and account-deletion requests
-- =========================
...
create index idx_user_requests_user_type_status_created
on user_requests(user_id, request_type, status, created_at desc, id desc);

create index idx_user_requests_operator_queue
on user_requests(status, created_at, id);

create unique index uq_user_requests_active_account_deletion
...
create table user_request_status_logs (
  id uuid primary key default gen_random_uuid(),
...
create index idx_user_request_status_logs_request
on user_request_status_logs(user_request_id, created_at, id);

create index idx_user_request_status_logs_admin
