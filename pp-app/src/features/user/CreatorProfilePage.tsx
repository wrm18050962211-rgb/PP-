import { ArrowLeft, Camera, Grid3X3, Heart, MessageCircle, Repeat2 } from 'lucide-react';
      ...
      <section className="px-4 pb-4 pt-3">
        <div className="flex items-center gap-5">
          ...
          <div className="grid min-w-0 flex-1 grid-cols-3 gap-2 text-center">
            ...
            <ProfileStat value={photographers.length} label="摄影师" />
            <ProfileStat value={120 + works.length * 8} label="关注" />
          </div>
        </div>
...
function ProfileTabs() {
  return (
    <div className="grid grid-cols-4 border-y border-white/10 text-white/60">
      ...
      <button className="flex h-12 items-center justify-center" aria-label="视频">
        <Camera size={21} />
      </button>
      <button className="flex h-12 items-center justify-center" aria-label="转发">
        <Repeat2 size={21} />
      </button>
      <button className="flex h-12 items-center justify-center" aria-label="喜欢">
        <Heart size={21} />
      </button>
    </div>
  );
