...
export function registerWithPhone(input: RegisterInput) {
  ...
  const account: AuthAccount = {
    ...
    roleReviewStatus,
    nickname: input.role === 'companion' ? 'Demo Studio' : 'Demo Client',
    creatorName: input.role === 'consumer' ? existing?.creatorName || 'Demo Client' : existing?.creatorName,
...
export function completeRoleRegistration(role: PublicRole, profile: Partial<Pick<AuthAccount, 'creatorName' | 'photographerName' | 'creatorAvatarUrl' | 'photographerAvatarUrl'>> = {}) {
  ...
  const nextAccount: AuthAccount = {
    ...
    creatorName: profile.creatorName ?? (role === 'consumer' ? account.creatorName || 'Demo Client' : account.creatorName),
...
function readAccount(): AuthAccount | null {
  ...
  try {
    ...
    const sanitized = sanitizeLegacyAccount({
      ...
      roleReviewStatus,
      nickname: account.nickname || (account.role === 'companion' ? 'Demo Studio' : 'Demo Client'),
