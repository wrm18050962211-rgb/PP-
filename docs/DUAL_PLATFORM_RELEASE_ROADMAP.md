...
界面基线固定为 `docs/MAIN_APP_UI_BASELINE.md` 记录的既有主 App：黑底双列作品流、黑底沉浸式作品详情、浅色“我的”和悬浮黑色底栏。Store Lite 的独立编译入口只用于在构建期排除未获准能力，不代表另做一套产品视觉；消费者首审页面必须沿用这套视觉语言和导航结构。默认 `build`/`preview`/iOS Debug 用于主 App 视觉延续，正式首审仍必须通过显式 `build:store-lite`、`ios:sync:store-lite` 和 Release 守卫。完成旧版视觉下的 Store Lite 路由白名单前，不得把主 App Debug 包当作 App Store 候选包。
...
- 独立的 Store Lite 编译入口和路由白名单，复用主 App 视觉基线但在编译期排除范围外模块；生产包不依赖 mock、localStorage 或运行时开关隐藏未完成页面。
