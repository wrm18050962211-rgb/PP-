...
7. 摄影师质量分：响应率、完单率、取消率、评分、举报扣分。
8. 功能开关和 A/B 实验。
9. 数据足够后再做协同过滤、学习排序和 AI 偏好理解。
...
#### AI V4：有限照片理解和个人摄影 Skill 版

目标：在偏好档案已有真实使用数据后，用用户主动选择的少量图片提高理解效率，形成可持续更新的个人摄影偏好 Skill。

任务：
1. 分析用户主动选择的少量喜欢、不喜欢和参考图片。
2. 结合问卷、收藏、订单快照和拍后反馈生成可解释观察。
3. 通过追问区分“过去经常这样拍”和“用户真正喜欢这样拍”。
4. 按日常、旅行、纪念日、情侣/家庭等场景保存偏好版本和置信度。
5. 用户确认后才更新长期档案。
...
- 每条建议有来源、场景、置信度和确认记录。
- 相比纯问卷版本，能证明减少沟通或提高满意度，否则不扩大图片处理范围。
...
6. 客服 RAG。
7. AI 审核和举报摘要。
8. AI 搜索、地点方案、创作者成长助手和运营日报。
9. 在偏好档案已有真实使用数据后，试点用户主动选择的少量照片分析。
10. 数据证明有效后形成分场景的个人摄影 Skill。
11. 最后再做轻量 agent 工作流。

这个顺序的核心判断：先把用户确认过的结构化偏好变成真实业务数据，再用 AI 降低表达和理解成本；先做少量、自主、可解释的图片分析，再考虑更复杂的个人摄影 Skill 和 agent 化。AI 不能成为偏好档案上线的前置依赖。
