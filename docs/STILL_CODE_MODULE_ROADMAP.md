...
这份文档用于指导当前代码从“可演示 MVP”分两步推进：先完成不含支付的 `Store Lite 1.0` 并取得公开 App Store 页面，再完成可真实收费运营的商业 `v1.1`。它整合前端、后端、数据库、支付、地图定位、运营后台、合规、监控、推荐与增长模块。

当前结论：项目已经有较完整的业务雏形，但仍有大量模块停在 mock、localStorage、JSON store、演示数据或半接入状态。首次上架优先级不是继续堆新页面，而是把消费者专用构建、真实精选内容、无支付预约申请、账号删除、举报客服、生产数据和审核材料做稳。支付、完整地图、聊天、媒体上传和收费运营基础设施在取得公开页面后继续完成。

当前发布范围以 `DUAL_PLATFORM_RELEASE_ROADMAP.md` 第 0.13 节为准。本文件后文保留的支付、地图、聊天、媒体上传、完整后台和完整运维清单，是商业 `v1.1+` 的建设清单；除非明确写入 Store Lite 节点，不得再把它们解释成 Apple 首审依赖。
...
## 1. 分层门槛：Store Lite 首审与商业运营

Store Lite 首审只取本章中的生产配置、消费者账号、真实精选内容、无支付预约、合规入口、移动稳定性和适用备案；完整支付、地图、上传、聊天、Admin、监控和恢复演练继续作为商业收费前门槛。
...
Store Lite 的上线前硬门槛是：任何 Admin/运营代码都不能进入 App Store 移动端包，预约、举报、客服和删除申请必须由受保护、可审计的窄运营入口处理。完整 Web Admin 的模块化、独立域名和正式独立部署属于商业 v1.1/P1，不反向阻塞首次上架。

代码事务：

- Store Lite 先提供只覆盖预约、精选内容、举报、客服和删除申请的窄运营入口；完整后台短期可用 `pp-app/src/app/AdminApp.tsx` 或后续 `apps/admin` 承载。
- 移动端入口只挂载 Client/Photographer 路由，不再包含 `/admin`、`AdminDashboard`、后台 mock 数据或后台登录页。
- 窄运营入口和后续完整后台只挂载受保护运营路由，不复用移动端 tab、个人中心、设置页返回逻辑。
- 后台使用独立 session key、admin token、admin API client 和权限判断。
- 生产移动端构建必须能通过环境变量或独立入口排除 admin bundle。
- 商业 v1.1 前，完整后台生产构建必须独立部署到内部门户或受保护域名；Store Lite 的窄运营入口可以先部署在受保护的内部路径，但绝不通过 App Store 分发。
...
- 访问窄运营入口或完整后台必须单独登录受限运营/管理员身份。
- 后台退出不会改变 Client/Photographer 的移动端登录态。
- 管理员敏感操作均写入审计日志。

### 1.3 PostgreSQL 真写入

Store Lite 子集只要求消费者账号、精选内容、摄影师资料、预约申请/状态、客服、举报和账号删除真实落库。订单、支付、聊天、退款和媒体写入属于商业 v1.1/P1 增量，不阻塞首审。

代码事务：

- 完成 `server/store/postgresStore.mjs` 写入能力。
- 将 Store Lite 的预约、举报、客服和删除申请写入路径接到 PostgreSQL transaction；后续再接订单、支付、消息和退款。
...
- Store Lite 预约创建、确认/拒绝、取消和状态日志真实写入并可跨设备恢复。
- 举报、客服和删除申请真实写入并可由受保护运营入口处理。
- 商业 v1.1 再验收 `orders/payments/order_status_logs/availability_slots`、支付回调和聊天写入。

### 1.4 订单并发与幂等

Store Lite 首审只验收预约申请的重复提交、确认/拒绝和取消幂等。以下 slot、订单、支付、退款和资金状态幂等属于商业 v1.1，不阻塞首次上架。
...
- App 内可以发起删除账号。
- App 内可以举报作品、摄影师或相关预约问题。
- 后台能看到举报并处理。
- App Review 账号能跑通主流程。
- 支付说明、退款说明和聊天举报在对应功能进入商业构建时再补，不在 Store Lite 展示占位入口。
...
这一节属于商业 v1.1/P1 的收费运营放行门槛，不反向阻塞 Store Lite 首审。Store Lite 仍必须具备正式 HTTPS、最小权限、自动备份、健康检查、request ID、版本映射、应用回滚和明确负责人；外部监控、完整崩溃诊断、异地恢复演练、预算/配额告警、证书多阶段提醒和主备事故响应在恢复支付前全部完成。
...
### 2.7 妆造供给与组合订单：需求卡快选，服务项独立

本节是摄影订单内的窄范围 P1 冷启动试点，不代表提前建设泛商家或本地生活市场。妆造供给同时允许个人妆造师与妆造门店参与，执行节点和状态以 `DUAL_PLATFORM_RELEASE_ROADMAP.md` 的 `WIN/IOS/INT/EXT-MERCHANT-*`、`WIN/IOS/INT-COMBO-*` 为准。本节记录 2026-08-25 已确认的新产品决策；`STILL_MERCHANT_COLD_START_PARTNERSHIP_PLAN.md` 已同步升级为 1.1，现有 Word/PPT 合作材料仍需按新方案重写后才能用于试点批准。

术语固定为“妆造”，基础含义是“妆面＋发型或基础整体造型”。服装租赁、配饰、复杂造型、多人妆造和全程跟妆必须作为明确包含项或独立加项展示，不得只用“造型”一词让用户误以为全部包含。

#### 2.7.1 用户入口与需求卡

- 用户仍从作品或摄影师进入，不增加“找妆造”底栏、首页入口或独立一级市场；妆造是当前拍摄需求卡中的可选服务。
- 需求卡先展示由当前作品、摄影师套餐和常见需求生成的极简推荐方案，只要求用户优先确认时间、拍摄地点和人数；成片数量、时长、场景、路线等已有建议直接预选，特殊项收进“更多需求”。
- 推荐方案必须先展示“你将获得什么”、摄影与妆造服务清单和预计总价，不能让用户先读完一组配置项才知道最终收获。
- 妆造区域先显示“不需要 / 添加妆造”。点击“添加妆造”后在需求卡内轻量展开“平台推荐 / 个人妆造师 / 妆造门店”，不强制跳转到新页面。
- 首屏每类最多展示 3 个候选。每张候选卡展示 1—2 个真实作品预览、名称、擅长风格、上门/到店、标准时长、固定价格和一句路线结论；点击作品可看大图，只有主动点击“查看更多”才进入完整作品列表。
- 候选卡支持单击选择，选择后立即回到需求摘要并更新总价、准备时间和衔接提示。移动端不得把完整商家资料、长说明和全部筛选项堆进需求卡。

#### 2.7.2 快速下单与定制咨询

- 固定价摄影套餐、固定价妆造套餐、服务区域和期望时间均可判断时，用户可以从需求卡直接提交标准订单，跳过聊天和人工改价。
- “直接下单”不等于“立即确认”。没有实时锁定档期时，页面必须明确显示“已下单，待服务方确认”；只有服务方公开可售档期已被可靠锁定时才可使用“预约已确认”。
- 摄影师和妆造方不得在接单时重写用户已确认的标准套餐价格，只能接受或按结构化原因拒绝。
- 复杂造型、多人服务、非标准服装、超服务区、无法锁价或用户主动填写定制要求时，主按钮切换为“提交需求，等待确认”，继续使用咨询/报价流程。
- 关闭旧 `CheckoutPage` 的无结构直接支付遗留路径，但保留本节定义的“结构化标准方案直接下单”和“非标准方案咨询报价”双路径；两条路径最终使用同一订单、服务项、支付、退款和结算事实源。

#### 2.7.3 定位与推荐规则

- 摄影师以用户计划拍摄地点为主要地理锚点，优先考虑服务范围、对该地点的经验、作品风格、档期和价格。
- 妆造以用户拍摄前的暂居地为主要锚点，例如家或酒店。匹配阶段只使用区域或模糊位置，订单正式确认前不得向个人妆造师或门店返回精确房号、住址或非必要联系方式。
- 个人妆造师优先计算其前往用户暂居地的上门距离、服务区域、可达时间、风格匹配、履约率和价格。
- 妆造门店不能只按“离用户最近”排序，应估算“暂居地 → 门店 → 拍摄地”的总绕行和妆造后到拍摄地的衔接风险，并显示“顺路 / 轻微绕行 / 时间偏紧”等用户可理解结论。
- 排序第一版使用可解释规则，不训练复杂模型。平台推荐必须保留个人与门店的供给多样性，不能因为距离权重让单一类型长期占满首屏。

#### 2.7.4 供给、作品与领域边界

- 个人妆造师和妆造门店都是独立服务提供方，分别承担身份/主体核验、档期、价格、接单、履约、退款、争议和结算责任。
- 作品必须与实际履约主体对应。个人妆造师只能使用本人可验证作品；门店随机分配人员时只能标为门店/团队风格，展示特定妆造师作品且承诺该效果时必须支持指定并锁定该妆造师。
- 妆造/服装不是 `order_extras`：它有独立提供方、时间、接单、履约、退款和结算责任。父订单保持一份清单、一个总价和一次支付，内部以 `order_items` 或等价服务项表达。
- 现有纯摄影订单兼容为一个摄影服务项，功能开关关闭时行为完全不变；功能开关开启时，父订单和服务项在支付后待接单、确认、完成、取消、退款和争议状态上原子同步。
- 先完成统一妆造供给领域骨架，再做候选推荐和需求卡快选，最后在支付、退款和结算基础上接真实组合交易。`INT-COMBO-1` 前禁止真实组合支付，不得用整单退款模拟部分退款，也不得把个人妆造师或门店伪装成摄影师钱包。
- 每服务项独立结算和争议冻结；只有责任无法拆分、支付异常或整单欺诈风险时才冻结整单。

最小领域对象：

- `merchants`：门店主体、公开资料和非公开营业电话；
- `styling_provider_profiles` 或等价统一读模型：以 `individual / merchant` 区分个人妆造师和妆造门店，并保存审核、服务方式、服务区域与履约状态；
- `styling_offerings` 或对现有 `merchant_offerings` 的兼容扩展：保存固定价、版本化的妆造套餐、上门/到店方式、时长、包含/不包含项和适用人数；
- `styling_works` 或等价作品关系：保存作品授权、实际履约主体、风格标签和展示范围；
- `photographer_styling_links` 或对现有合作关系的兼容扩展：表达双方确认的主要合作与可搭配关系，但不把妆造方所有权硬编码到摄影师账户；
- `order_items`：摄影、妆造和服装服务项的提供方、价格/内容/时间/地点快照及状态；
- 后续由组合交易节点扩展每服务项退款、结算和账本，而不是一次完成高风险重构。

验收至少覆盖：需求卡默认值与折叠项、1—2 张作品预览、个人/门店切换、标准方案直接下单、定制咨询回退、暂居地隐私、两类距离排序、作品归属、历史订单回填不改金额/状态、关系唯一性、价格快照、父子生命周期同步、最后服务项保护、联系方式脱敏、服务项权限、迁移幂等、schema parity、PostgreSQL mapper/read model 和纯摄影回归。
...
- 用户可选上传少量喜欢、不喜欢和参考图片；上传后只展示“动作”“构图/机位”“光线/色调”“氛围感”“都喜欢”等点击即保存的一键多选标签，标签可跳过，不弹强制二级问卷。
- 参考图和标签默认仅用于当前订单；方案确认或订单完成后再用独立的一键入口询问是否保存为长期偏好，不在每次上传时增加用途选择。
...
本次拍摄需求
  ...
  styles
  reference_media_ids
  reference_focus_tags    # 动作 / 构图与机位 / 光线与色调 / 氛围感 / 都喜欢；可空
  reference_usage_scope   # 默认 current_order；确认后才可 promoted_to_profile
  wanted_results
  avoid_results
  route_advice_needed
...
- 当前订单参考图、快速标签和明确要求的优先级高于长期偏好及模型推断；单次标签未经独立确认不得写入长期档案。
...
- 参考图上传后的标签完成率、平均点击次数和完成耗时；不能为了提高采集量增加强制确认页或二级问卷。
...
- 快速标签可一键多选、可跳过、点击即保存；参考图默认仅用于当前订单，长期沉淀有独立确认记录。
...
- 忠诚度功能不能新增一条平行交易链路。必须复用同一交易事实源：标准方案走“需求卡 → 直接下单 → 服务方确认 → 支付/履约 → 双方确认 → 平台结算”，非标准方案走“需求卡 → 咨询报价 → 用户确认 → 支付/履约 → 双方确认 → 平台结算”。
- 当前 `CheckoutPage` 的无结构直接全额支付遗留路由应关闭或重定向；需求卡中的结构化标准方案快捷下单必须保存相同的需求快照、服务项、佣金和权益数据，不能形成第二套订单模型。
...
目标：Store Lite 可审、可提交 TestFlight、可跑真实无支付预约闭环。

1. 冻结第一版范围：精选作品、找摄影师、详情、手填地点、预约申请、预约状态、客服/举报、我的和账号删除。
2. 改生产配置：`VITE_API_BASE_URL` production 必填，生产禁用 mock fallback。
3. 建立 Store Lite 专用构建入口：编译排除摄影师端、Admin、支付、社区、地图 SDK、上传和泛私信。
4. 接真实消费者登录：优先完成现有手机号验证码，并在首发 territory 固定前确认 +86 限制与目标用户一致；否则补 E.164 或经批准的替代登录。
5. 后端改 session/token：替换 `activeSession` 全局会话。
6. 接 PostgreSQL 真实内容与预约：创建、查看、取消、跨设备恢复和幂等；由平台受保护运营入口线下联系摄影师后确认/拒绝，并保存用户可见到店说明和平台客服。
7. 建立最小受保护运营入口：维护精选内容、处理预约、客服、举报、下架和删除申请；禁止运营直接修改生产数据库。
8. 补 App Store 合规入口：隐私、协议、客服、举报、屏蔽、数据权利申请和删除账号；自动导出是否首审实现以正式合规结论为准。
9. 补移动端错误态：loading、empty、error、retry；生产失败不能伪装空状态或成功。
10. 准备无管理权限、无真实隐私数据、服务端受限且可撤销/轮换的 App Review 主备账号、四种预置预约状态、审核说明和构建排除检查；凭据只填写在 App Store Connect 的 App Review Information/Notes，不得写入二进制或公开文档。
11. 并行业务侧准备：
    - 确定主云服务商和公司主体云账号。
    - 注册公司主体域名并完成实名认证。
    - 准备官网、隐私政策、用户协议、客服和删除账号说明。
    - 若首发包含中国大陆，完成适用的 ICP、APP 备案、展示和其他资质，或取得正式“不适用”结论。
    - 若备案流程必须先有公开 App Store 页面，首发 territory 暂时排除中国大陆，上架后继续备案并在完成前不开放大陆。
    - 记录首发 territory，确认登录方式适用于目标用户，并确认支付渠道是否接受该公开页面。
12. 完成 Store Lite 最小运维：正式 HTTPS、最小权限、自动备份、健康检查、request ID、版本映射、应用回滚和一名首发负责人。

支付商户入网、支付/退款、地图企业认证、用户媒体上传、完整监控/恢复演练和双边交易均移至上架后的商业阶段。
13. 跑最小验证：
...
4. 隔离管理能力：Store Lite 移动端生产包编译排除 `/admin`，先接受保护的窄运营入口；完整 Admin 的独立构建和部署在商业 v1.1 前完成。
...
这一节主要是 App Store 上线后的工程升级路线。注意：管理代码从移动端包中编译排除、窄运营入口受保护且可审计已经前移到 `1.2A`，属于 Store Lite 硬门槛；完整 `apps/admin`、独立域名、共享包抽取和部署体验完善属于商业 v1.1/P1。
...
前端交互：
- Agent 后置为“帮我整理需求/帮我选”，不作为首页第三个搜索入口。
- 解析后展示筛选 chips，让用户确认。
- 用户确认后再调用真实匹配接口。
- Agent 和手动菜单读写同一份结构化需求草稿，用户可以逐项修改或删除。

验收标准：
- AI 只负责解析，不直接创建订单。
- AI 不自动付款、改价、取消、替服务方接单；没有真实库存时只能生成期望时间，不能承诺预约成功。
- 解析失败时回退普通筛选。
- 推荐理由不能编造不存在的摄影师能力。
...
输入来源：
- 当前订单主动上传的参考图、快速标签和明确要求，优先级最高。
- 当前订单的场景、风格、地点、时间和互动需求。
- 用户在 Still 内主动标记喜欢或不喜欢的作品及原因。
- 用户明确填写并确认过的长期偏好；与本次需求冲突时以后者为准。
- 摄影师的服务范围、设备、可用焦段和已声明能力，仅用于过滤不可执行方案。
...
- 参考图上传阶段默认仅用于本次拍摄；方案确认或订单完成后，用户才可以对系统总结选择“加入长期偏好”“仅用于本次拍摄”“不是”“暂不确定”。
...
- 根据用户场景推荐地点。
- 根据摄影师服务半径解释是否可服务。
- 结合本次参考图/快速标签、明确要求、喜欢/收藏、已确认长期偏好和摄影师能力生成构图、静态姿势、人物站位和镜头焦段参考卡。

硬约束与偏好优先级：

```text
先满足：当前订单地点、时间、场景、安全限制、摄影师设备与服务能力
再排序：当前订单参考图、快速标签和明确要求
  > 用户主动喜欢/收藏
  > 用户确认过的长期偏好
  > 通用模板
```

- 本次订单的明确需求必须覆盖历史偏好和 AI 推断。
- 任何参考图或历史偏好都不能覆盖地点开放条件、现场安全、拍摄时间或摄影师明确声明的设备/能力限制。
- 每个方案显示简短生成依据，例如“参考图 1 的动作 + 订单 17:30 光线 + 暖色偏好”。
- 方案保存 `input_version` 和订单快照；地点、时间、参考图或明确要求改变后，旧方案标记“订单信息已变化”，由双方主动重新生成，不能静默修改已确认方案。

订单内导演模式：
- 文字步骤始终弹出并作为事实源；每步只包含一个主要动作，支持重复、下一步、跳过和完成。
- 未连接耳机时语音默认关闭；连接耳机且用户主动进入导演模式时默认开启，用户手动静音后保持静音。
- 耳机中途断开时立即停止当前语音，不得自动切换到手机扬声器；文字引导继续工作。
- 语音只播放已生成的步骤，不声称已经看见或判断现场画面。
...
- 第一版不做现场试拍图上传分析；用户和摄影师先依靠方案预览、三条以内常见问题和自身判断调整，避免引入相机传图、媒体权限、等待和模型成本。
...
- 快速标签可跳过且无需确认页；地点/时间/参考图改变后旧方案有过期提示和手动重新生成入口。
- 文字步骤在静音状态下完整可用；耳机连接、手动静音和中途断开行为通过真机验证，任何情况下都不能意外外放。
- 当前范围不包含试拍图片分析、持续摄像头识别、相机品牌传图或自动连拍。
...
任务：
1. AI 搜索意图。
2. 基于订单地点/时间、快速标签、参考图和确认偏好的 AI 方案清单，以及文字优先、耳机语音可选的导演模式。
3. AI 创作者成长建议。
4. AI 运营日报。
...
- 地点/时间/参考图变化可识别方案过期；无耳机时不默认外放，耳机断开立即停止语音。
- 不以现场试拍图分析、持续摄像头识别或相机传图作为本版本验收项。
- 运营日报可跳转到真实业务对象。
...
#### 候选研究：现场试拍图分析与连续实时视觉（不进入当前版本承诺）

当前不为现场试拍图上传分析、相机品牌传图、实时取景框、人物站位识别、连续姿势纠正或自动连拍建立开发节点。摄影师和用户本身可以对照方案预览与模板进行现场调整，新增视觉分析可能带来传图链路、媒体权限、等待、模型成本和拍摄中断。只有真实使用数据证明方案清单与导演模式仍无法解决高频问题，摄影师主动表达上传试拍图的需求，且设备链路和效率收益可验证时，才单独重新评审；拍后质检或后处理 Copilot 通过不自动解锁本能力。
...
6. 客服 RAG。
7. AI 审核和举报摘要。
8. AI 搜索、订单绑定的方案清单、文字优先/耳机语音可选的导演模式、创作者成长助手和运营日报；拍摄参考可调整或忽略。
...
13. 再做轻量 agent 工作流。
14. 现场试拍图分析、相机传图、实时取景和连续姿势指导只保留为候选研究，不进入当前版本承诺。

这个顺序的核心判断：先用可跳过的一键标签获得低成本、明确的用户输入，再把订单地点/时间、参考图和偏好转成可执行但不强制的方案；现场以文字为主、耳机语音为辅，不为试拍传图和实时视觉提前增加工程范围。拍摄后优先解决摄影师真实耗时，再用确认过的结果形成个人摄影 Skill。AI 不能成为偏好档案上线或订单交付的前置依赖。

---

## 10. Still 产品定位升级：从陪拍交易到现实视觉体验

本节记录 Still 已确认的长期产品方向，但不改变第 1—4 节的当前上线优先级。当前版本先完成 Store Lite 的真实内容、无支付预约和 App Store 首审；支付与履约进入商业 v1.1。本节所有扩展必须在公开基线和后续数据闸门后逐阶段启动，不能反向阻塞首审、支付审核、履约、合规和基础运营。

### 10.1 最终产品定位

对用户的一句话表达：

> 发现值得去的地方，安排适合你的体验，把它好好留下来。

产品品类定义：

> Still 是一个内容驱动的城市与旅行视觉体验平台，帮助用户从“看到心动”走到“真正到场、完成体验并留下记忆”。

内部能力链路：

> 发现 → 计划 → 体验 → 留下

摄影仍是 Still 的核心专业能力、可信内容来源和首个付费切口，但不再是唯一长期商品。Still 既服务人物拍摄，也服务风景、咖啡厅、餐厅、艺术社区、展览、街区、公园、建筑、活动、Citywalk、约会、城市周边和旅行等现实场景。摄影是让一次消费或出行变成个人记忆的方式，而不是限制内容边界的唯一分类。

Still 不对外宣传为“携程 + 美团 + 小红书 + 抖音”。它只吸收这些平台与现实体验有关的部分，并通过个人上下文把它们组织成一个可执行场景：

| 平台能力 | Still 吸收的部分 | Still 不复制的部分 |
| --- | --- | --- |
| 携程/飞猪 | 目的地、景点、路线、机酒门票跳转 | 完整 OTA 库存和交易后台 |
| 美团/点评 | 咖啡厅、餐厅、展览、本地活动和预约 | 外卖、团购和全部本地商户体系 |
| 小红书 | 图片、穿搭、地点经验、收藏和分享 | 全品类生活内容社区 |
| 抖音 | 短视频沉浸展示和低门槛兴趣激发 | 无限娱乐流和泛内容生态 |
| 地图平台 | POI、距离、路线和导航 | 自建地图、公共瓦片或导航系统 |

### 10.2 Still 的内容不是媒体库存，而是可执行场景

Still 的内容基本单位不是一张照片、一个视频、一个商户或一个摄影师，而是“用户能够完成的现实场景”。每个场景逐步结构化为：

- 人物、风景、地点和氛围内容；
- 精确地点与合法可用的地图入口；
- 推荐日期、时间、天气、光线和人流条件；
- 体验时长、预算区间和体力要求；
- 适合独自、朋友、约会、家庭或旅行的关系场景；
- 附近可组合的咖啡厅、餐厅、展览、活动或散步路线；
- Vibe 可调用的穿搭与携带物品建议；
- 机位、构图、动作和拍摄参考；
- 自助完成、找陪伴者或找专业摄影师的不同方案；
- 真实到访、成片、评价和记忆结果。

典型链路：

> 外部内容发现 → 进入 Still 场景 → 收藏/我想去 → 按我的情况生成计划 → 地图/预约/票务/陪拍履约 → 到访和拍摄 → 成片、回忆与分享

早期抖音、小红书、微博等外部平台承担主要拉新；Still 站内 Feed 承担兴趣承接、意图识别、计划和交易转化。站内加入内容流不等于天然拥有外部流量，不能把“做一个流媒体首页”当作获客方案本身。

### 10.3 内容与交易之间增加“计划层”

Still 不采用“刷到内容立即强推购买”的电视购物式路径。对非刚需、高信任的摄影和现实体验，正确的渐进关系是：

1. 内容先提供独立价值；
2. 用户主动表达收藏、想去或想拍；
3. 系统根据时间、位置、预算、天气、同行人和偏好进行适配；
4. 用户确认一个默认计划及少量备选；
5. 计划自然缺少某项能力时，再出现地图、票务、场馆、陪伴者或摄影师服务；
6. 用户在透明价格、规则和服务者信息下确认交易；
7. 完成结果回流到推荐、偏好、评价和个人记忆。

内容本身就是视觉化提示词。用户点击“按我的情况安排”后，系统已经知道场景、地点和氛围，只补问真正影响结果的一两个条件，避免从空白聊天框开始要求用户写完整需求。

### 10.4 内容商业化与用户信任边界

为了避免信息流变成推销，长期固定以下边界：

- 内容即使不产生订单，也必须提供真实地点、时间、路线、拍摄或体验价值；
- 用户未表达想去、想拍、咨询或计划意图前，不用高压购买按钮打断内容；
- 用户内容、平台策划、摄影师作品、商家合作和付费推广必须明确区分；
- 推荐排序优先考虑用户适配、距离、时间、真实完成率、作品结果、评价和价格透明度，不由佣金高低单独决定；
- 不使用虚假名额、倒计时、先涨后降、模糊总价或隐藏退改规则制造焦虑；
- 商业服务在计划和履约层透明出现，不伪装成普通用户分享；
- 当前阶段不依赖商家广告变现，避免在平台尚未建立信任时污染内容；
- 内容播放和停留不是最终成功指标，必须继续观察计划、到访、成交、完成和满意结果。

### 10.5 服务梯度与长期收入边界

Still 从单一摄影陪伴订单逐步形成三层服务，不要求每个内容消费者都预约摄影师：

| 服务层 | 用户获得的能力 | 优先商业模式 |
| --- | --- | --- |
| 自助模式 | 场景拍摄包、地点、路线、时间、机位、动作、照片故事、短视频分镜和 AI 适配 | 免费基础能力、单次付费场景包；验证后可进入会员 |
| 陪伴模式 | 当地陪伴者帮助带路、交流和拍照 | 平台服务抽成、定金和履约保障 |
| 专业模式 | 专业摄影、妆造、精修和主题体验 | 高客单服务抽成与专业模块收入 |

只有用户已经形成明确体验计划时，才逐步接入展览/活动门票、咖啡厅/餐厅预约、地图/旅行跳转、Vibe 穿搭、修图、照片打印和纪念产品。平台不为追求收入而在内容层堆叠无关商品。

### 10.6 自助模式：不是另一个相机，而是现实场景拍摄导演

自助模式不与系统相机竞争成像质量，不与抖音竞争特效、热点和内容分发，也不与剪映竞争完整视频剪辑。Still 需要拥有的是现实场景执行层：

> 告诉用户去哪里、什么时候去、穿什么、拍什么、怎样拍，以及怎样把一次普通出门完整留下来。

系统相机继续承担拍摄和原片保存；外部内容平台继续承担主要传播；通用修图和复杂时间线剪辑优先通过系统能力、合法跳转或成熟工具完成。Still 只建设能强化“场景理解—现场完成—结果回流”的差异化能力。

在自助模式得到真实使用数据之前，不把自研相机、通用滤镜商城、完整视频编辑器、全网热点聚合或持续实时视觉识别列入前置工程。

#### 10.6.1 核心商品：场景拍摄包

Still 的自助服务基本单位不是一个滤镜或一段音乐，而是一个可以在现实地点完成的“场景拍摄包”。一个标准场景拍摄包逐步包含：

- 精确地点、建议到达时间、天气、光线、人流和路线；
- 适合独自、朋友、情侣、家庭、三脚架或陪伴者的完成方式；
- 可由 Vibe 调用的穿搭、颜色、鞋包和携带物品建议；
- 5—10 张照片组成的照片故事，包括环境开场、人物、细节、互动、移动和结束镜头；
- 5—8 个短视频素材槽位，包括建议时长、景别、运镜、动作和拍摄顺序；
- 机位、构图、动作、参考结果和安全/场地规则提醒；
- 可选的场景专属调色、封面、文案和发布建议；
- 自助完成、预约陪伴者和预约专业摄影师的升级入口。

场景拍摄包必须首先解决“普通用户不知道拍什么、拍不完整”的问题。滤镜、模板和 AI 只作为降低完成难度的工具，不能替代真实地点、明确步骤和可验证结果。

#### 10.6.2 从“拍同款”扩展为“去同款—拍同款—做成同款”

Still 可以学习短视频平台用模板降低创作门槛的机制，但要把数字模板转化为现实体验：

1. **去同款**：告诉用户地点、到达时间、光线、人流、路线、预算和必要预约；
2. **拍同款**：提供照片清单、短视频分镜、具体机位、动作和逐项完成提示；
3. **做成同款**：把用户选择的照片和短片按场景结构整理，生成照片故事、短视频、封面和文案；
4. **分享结果**：用户可分享到抖音、小红书、微博或微信，分享入口回到 Still 的同一场景页；
5. **结果回流**：真实到访、完成率、保留结果和分享行为继续改善场景排序及个人偏好。

长期受控循环：

> 看到场景 → 去同款 → 拍同款 → 做成同款 → 分享 → 新用户进入同一 Still 场景

Still 不复制外部平台内容、音乐或特效资产。热点只作为可选的内容信号，不作为核心供给；音乐使用正版授权、原创素材、节拍占位或发布时到目标平台选择，避免建立不可控的版权依赖。

#### 10.6.3 照片与短视频的统一结果

自助模式不把照片和视频拆成两套彼此孤立的工具，而是围绕一次现实体验生成三种结果：

- **Still Photo Story**：由 5—10 张不同作用的照片组成一段视觉叙事，而不是只追求一张成功照片；
- **Still Short Story**：用户按 5—8 个素材槽位拍摄，系统按既定顺序组合为约 10—30 秒短视频；
- **Still Moment**：把照片故事、短视频、地点、同行人和当天感受沉淀为一张长期记忆卡片。

第一版现场指导以静态参考、文字步骤、倒计时、拍摄清单和“下一镜”为主。因为 Still 在拍摄前已经定义素材槽位，后续自动组合应优先做模板化排序、裁切和统一风格，不先建设能够处理任意长视频的通用 AI 剪辑器。

语音导演、构图叠层、实时姿势纠正、自动抓拍和连续视觉分析只作为后续候选能力。只有静态指导的完成率和用户反馈证明现场指导有稳定价值，才进入单独评审。

#### 10.6.4 免费、单次付费、会员与真人服务边界

固定商业化原则：

> 免费让用户完成一次合格体验；付费让结果更个性、更省事、更稳定、更专业。

| 层级 | 用户获得的能力 | 收入方式与边界 |
| --- | --- | --- |
| 免费基础 | 浏览/收藏场景，基础地点、时间、路线和安全信息，3—5 个照片机位，3—5 个短视频素材槽位，倒计时、拍摄清单、原片保存和基础导出 | 用于建立习惯和验证需求；不强制水印，不限制用户保存自己的原片 |
| 单次付费场景包 | 完整照片故事和短视频分镜，精确点位、季节/光线、动作参考、场景调色、封面与文案，摄影师或创作者的专业方法 | 单次购买并与合格创作者分成；不把单个普通滤镜包装成核心商品 |
| AI 会员 | 按用户风格、衣橱、天气、同行人、时间和设备调整方案；智能选片、统一风格、自动组合、长期偏好与私密记忆 | 在基础场景包已证明复用后测试订阅；AI 不作为免费体验能否完成的前置条件 |
| 真人服务 | 陪伴者、摄影师、妆造、精修、场地、门票或其他计划内服务 | 收取透明服务费、定金保障或交易佣金；只在用户计划确实缺少该能力时出现 |

基础拍摄、用户原片保存、基础导出、地点安全信息和少量可完成的拍摄步骤不得设置为付费墙。基础滤镜如提供应保持免费；真正适合收费的是场景专业知识、个性化适配、自动化处理、长期记忆和真人履约。

#### 10.6.5 自助模式子路线与验证指标

自助模式服从第 10.7 节的总阶段，不改变当前上线顺序：

1. **准备期（产品阶段 2）**：用人工策划把少量高质量内容整理成只读场景卡和基础拍摄清单；验证用户是否会收藏、想去、打开清单和真实到访，不开发自研相机；
2. **自助 V0（产品阶段 3）**：加入照片/视频清单、素材槽位、调用系统相机或导入素材、完成勾选和基础结果保存；
3. **自助 V1**：上线结构化场景拍摄包、照片故事、短视频模板化组合、结果回流和外部分享；
4. **自助 V2**：接入 Vibe、天气、同行关系和个人偏好，试点创作者付费场景包、智能选片、统一调色与会员；
5. **自助 V3 候选**：只有前三版数据证明必要时，才评审应用内引导相机、语音导演、构图叠层和受控实时视觉能力。

重点指标不是滤镜使用次数或视频播放量，而是：场景收藏率、拍摄清单打开率、清单开始率、镜头完成率、完整结果生成率、结果保留率、外部分享率、同一场景复用率、单次付费转化、会员留存、真人服务升级率和满意度。

以下任一情况出现时暂停扩大工程：用户只浏览但不启动清单；大量启动却无法完成；自动结果不如用户直接使用系统相机或现有剪辑工具；付费能力没有显著提升省事程度或结果质量；自助功能损害陪拍交易、内容信任或当前上线稳定性。

### 10.7 分阶段产品 Roadmap 与数据闸门

以下阶段以验证结果推进，不以日期自动解锁。每次进入下一阶段前必须重新估算开发量、运营供给、内容成本、合规风险和现金投入。

#### 产品阶段 0：Store Lite 首审先上线

目标：完成真实内容、摄影者、无支付预约申请、客服举报、账号删除和首审运营闭环，取得公开 App Store 页面。

当前范围：

- 使用现有真实客片、摄影者作品、地点和可预约参考内容；
- 保持当前 Feed 和页面主结构，不因长期定位进行大改；
- 建立内容来源、详情进入、预约提交和处理结果的最小可追踪记录；
- 优先解决生产数据库、稳定精选媒体、消费者登录、预约、合规、最小运营和基础备份；
- 支付、完整地图、聊天、媒体上传和完整运维留给上架后的商业版本；
- 不启动泛 UGC、短视频推荐算法、场馆招商或生活 Agent 新工程。

退出闸门：以 `DUAL_PLATFORM_RELEASE_ROADMAP.md` 的 P0、TestFlight 和 App Store 验收为准。

#### 产品阶段 1：验证陪拍交易与内容转化

启动条件：Store Lite 已公开，真实用户能够完成预约申请；支付渠道和商业 v1.1 另按唯一 Roadmap 验收。

目标：证明 Still 的真实客片、摄影者作品和地点内容可以产生摄影服务需求，并安全完成履约。

任务：

- 在一个城市聚焦少量高质量摄影者和标准场景；
- 区分外部内容、站内内容、主动搜索和老客复约带来的咨询/订单；
- 通过用户访谈理解“不咨询、不支付、取消和不满意”的原因；
- 继续优化作品真实性、摄影者匹配、需求确认、定金、服务恢复和成片反馈；
- 暂不扩展到泛本地生活或大型内容社区。

窄范围例外：若 `INT-MERCHANT-1`、`INT-COMBO-1` 和外部合规/支付闸门按唯一 Roadmap 完成，可以在摄影订单内受控试点 2—4 名个人妆造师和 2—4 家妆造门店。该例外只服务“一站式摄影履约”，不开放独立妆造首页、场馆/票务市场、全渠道库存或本地生活 Feed。

建议的首轮验证门槛：累计完成 20—30 笔真实订单，能够追踪内容来源，履约与售后没有系统性风险，并出现可重复的内容类型或场景。该数字是小样本产品判断门槛，不是市场规模或盈利证明。

#### 产品阶段 2：验证内容是否能创造现实需求

启动条件：阶段 1 证明交易成立，并且内容来源和转化漏斗可追踪。

目标：证明用户不仅在已经想拍照时搜索摄影师，也会因为看到真实场景而产生收藏、想去、计划和咨询。

任务：

- 以人工策划和现有供给制作少量高质量结构化场景，不先建设泛 UGC；
- 从旅行陪拍逐步加入咖啡厅、艺术社区、展览、街区、公园、建筑和 Citywalk；
- 提供收藏、想去、轻量计划和地图跳转；
- 用规则和人工运营先验证场景适配，不把复杂 AI 作为前置依赖；
- 找出至少三类能够重复带来计划、咨询、到访或交易的内容；
- 外部平台继续负责主要拉新，站内验证承接和转化效率。

核心漏斗：

> 合格内容曝光 → 收藏/想去 → 生成或查看计划 → 咨询/外部履约 → 支付 → 完成体验 → 成片/反馈/分享

核心指标按漏斗查看：收藏率、计划进入率、咨询率、定金转化率、订单完成率、满意度、内容来源订单占比和分享/推荐；播放量、点赞和停留只作为诊断指标。

#### 产品阶段 3：形成视觉体验服务梯度

启动条件：阶段 2 证明日常和旅行场景均存在“内容 → 计划 → 行动”，且用户不预约摄影师时仍需要 Still 的计划价值。

目标：从单一陪拍扩展为自助、陪伴和专业三层视觉体验。

任务：

- 按第 10.6 节的边界上线自助 V0/V1：地点、路线、机位、动作、照片故事、短视频素材槽位和拍摄清单；
- 优先调用系统相机和素材导入，不把自研相机、通用滤镜商城或完整视频编辑器作为阶段 3 前置条件；
- 在受控场景中接入 AI 个性化计划和 Vibe 穿搭；
- 试点单次付费场景包；基础完成能力保持免费，会员只承接个性化、自动化与长期偏好；
- 保留陪伴者撮合，增强低压力拍照、带路和交流价值；
- 为高需求场景提供专业摄影、妆造、精修和主题服务；
- 只在计划需要时跳转或接入地图、票务、餐厅和场馆服务；
- 比较三层服务的使用频率、获客成本、毛利和满意度。

进入下一阶段前，至少要证明一个非旅行日常场景可以稳定复用，并证明新增服务没有损害内容信任和基础摄影交易。

#### 产品阶段 4：形成城市视觉体验市场

启动条件：内容质量、真实到访、服务完成、复购、创作者供给和单位经济已经得到验证。

目标：形成“内容 → 计划 → 到访/交易 → 结果 → 新内容”的受控平台循环。

任务：

- 开放有限 UGC、真实到访和订单成片分享；
- 建立创作者、摄影者、场馆和商家的透明身份与内容来源；
- 建立地点、场景、摄影者和服务的分层评价；
- 试点场馆、活动、票务和预约能力，不自建已有平台的完整基础设施；
- 用真实行为和结果训练推荐，不只优化点击和观看；
- 单城市模型成立后再复制到其他城市。

该阶段不以内容数量或城市数量作为唯一扩张依据。无法维持内容真实性、服务质量、审核和单位经济时停止扩张。

#### 产品阶段 5：成为个人生活 Agent 的体验与记忆模块

启动条件：Still 已具有稳定的用户偏好、现实到访、服务结果和记忆数据，关系型个人生活 Agent 的入口价值已由独立原型验证。

目标：用户不必每次打开 Still 主动搜索，系统可以在授权下结合天气、位置、时间、预算、同行人、历史收藏和 Vibe 主动准备现实体验。

典型能力：

- 识别周末、约会、旅行、纪念日和临时空闲；
- 主动建议一个默认场景和一个有明确差异的备选；
- 联动 Vibe、地图、本地生活和旅行服务；
- 由用户确认后完成预约、跳转或交易准备；
- 把到访、照片、人物和感受沉淀为长期生活记忆。

该阶段属于母产品长期路线，不代表现在启动第三个大型 App，也不把工作、办公、代码或教育 Agent 纳入 Still。

### 10.8 当前明确不做的产品决策

本节只确认定位、范围、内容—计划—交易关系、信任边界、服务梯度和阶段目标，不确认最终页面结构。以下事项留到当前内容上线并获得真实行为数据后单独评审：

- 是否采用全屏短视频、图片流、双列卡片或混合内容形态；
- 首页中工具入口、内容入口和 AI 入口的视觉优先级；
- 底部导航名称、数量和信息架构；
- 内容详情、计划页和交易页的具体跳转方式；
- Feed 默认打开条件及是否按任务型/灵感型会话切换。

在另行评审前，不得引用本节作为重做当前页面或扩大当前版本范围的依据。
