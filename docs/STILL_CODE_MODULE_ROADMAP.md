...
当前发布范围以 `DUAL_PLATFORM_RELEASE_ROADMAP.md` 第 0.12 节为准。本文件后文保留的支付、地图、聊天、媒体上传、完整后台和完整运维清单，是商业 `v1.1+` 的建设清单；除非明确写入 Store Lite 节点，不得再把它们解释成 Apple 首审依赖。
...
### 2.7 妆造商家与组合订单：先建服务项，再开放资金链

本节是摄影订单内的窄范围 P1 冷启动试点，不代表提前建设泛商家或本地生活市场。执行节点和状态以 `DUAL_PLATFORM_RELEASE_ROADMAP.md` 的 `WIN/IOS/INT/EXT-MERCHANT-*`、`WIN/IOS/INT-COMBO-*` 为准，产品与合作规则见 `STILL_MERCHANT_COLD_START_PARTNERSHIP_PLAN.md`。

固定技术边界：

- 用户仍从作品或摄影师进入，不新增商家搜索入口；商家名称只筛选真实合作摄影师；
- 商家妆造/服装不是 `order_extras`：它有独立提供方、时间、接单、履约、退款和结算责任；
- 父订单保持一份清单、一个总价和一次支付，内部以 `order_items` 或等价服务项表达；
- 现有纯摄影订单兼容为一个摄影服务项，功能开关关闭时行为完全不变；
- 功能开关开启时，纯摄影服务项必须在原订单事务内同步支付后待接单、确认、完成、取消、退款和争议状态，禁止父订单与服务项状态分叉；
- 先完成领域骨架，再做商家/套餐/合作搜索，最后在支付、退款和结算 P0 基础上接组合交易；
- `INT-COMBO-1` 前禁止真实组合支付，不得用整单退款模拟部分退款，也不得把商家伪装成摄影师钱包；
- 正式确认前由服务端脱敏商家营业电话，前端隐藏按钮不是唯一保护；
- 每服务项独立结算和争议冻结；只有责任无法拆分、支付异常或整单欺诈风险时才冻结整单。

最小领域对象：

- `merchants`：商家主体、门店公开资料和非公开营业电话；
- `merchant_offerings`：固定价、版本化的妆造/服装套餐；
- `photographer_merchant_links`：摄影师—商家双方确认关系和唯一主要关系；
- `order_items`：摄影、妆造和服装服务项的提供方、价格/内容/时间快照及状态；
- 后续由组合交易节点扩展每服务项退款、结算和账本，而不是一次完成高风险重构。

验收至少覆盖：历史订单回填不改金额/状态且不与既有 `item_no` 冲突、关系唯一性、摄影套餐归属、价格快照、父子生命周期同步、最后服务项保护、电话脱敏、服务项权限、迁移幂等、schema parity、PostgreSQL mapper/read model 和纯摄影回归。
...
| 专业模式 | 专业摄影、化妆、造型、精修和主题体验 | 高客单服务抽成与专业模块收入 |
...
| 真人服务 | 陪伴者、摄影师、化妆、造型、精修、场地、门票或其他计划内服务 | 收取透明服务费、定金保障或交易佣金；只在用户计划确实缺少该能力时出现 |
...
窄范围例外：若 `INT-MERCHANT-1`、`INT-COMBO-1` 和外部合规/支付闸门按唯一 Roadmap 完成，可以在摄影订单内试点 1—3 家妆造/服装商家。该例外只服务“一站式摄影履约”，不开放商家首页、场馆/票务市场、全渠道库存或独立本地生活 Feed。
