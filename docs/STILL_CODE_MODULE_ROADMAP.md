...
本节记录 Still 已确认的长期产品方向，但不改变第 1—4 节的当前上线优先级。当前版本仍先完成真实摄影陪伴交易和内容上线；本节所有扩展必须在上线后按数据闸门逐阶段启动，不能反向阻塞 App Store、真实支付、履约、合规和基础运营。
...
目标：完成现有内容、摄影者、咨询、订单、支付、履约、售后和运营的生产闭环。

当前范围：

- 使用现有真实客片、摄影者作品、地点和可预约参考内容；
- 保持当前 Feed 和页面主结构，不因长期定位进行大改；
- 建立内容来源、详情进入、咨询、订单和完成结果的基础埋点；
- 优先解决生产数据库、对象存储、地图、支付、合规、后台、监控和备份；
...
启动条件：当前内容已经上线，真实用户能够完成咨询和交易，生产主流程稳定。
